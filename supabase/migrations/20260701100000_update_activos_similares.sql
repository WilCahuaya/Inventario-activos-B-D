-- Actualización masiva de ejemplares similares (misma plantilla que alta masiva).
...
DECLARE
  ...
  v_forbidden TEXT[] := ARRAY[
    ...
    'foto_path', 'comprobante_path', 'comprobante_serie',
...
BEGIN
  ...
  IF v_es_contador THEN
    IF v_template.estado_registro = 'PREREGISTRADO' THEN
      v_allowed := ARRAY[
        'codigo_catalogo', 'nombre', 'nombre_etiqueta', 'descripcion', 'caracteristicas',
        'categoria', 'estado_bien', 'marca', 'modelo', 'color', 'medidas',
        'medida_largo', 'medida_ancho', 'medida_altura', 'depreciacion', 'observacion',
        'valor_adquisicion', 'valor_es_mercado', 'fecha_adquisicion', 'vida_util_meses',
        'posible_ambiente_id'
      ];
    ELSE
      v_allowed := ARRAY[
  ...
  ELSE
    IF v_template.estado_registro = 'PREREGISTRADO' THEN
      v_allowed := ARRAY[
        'codigo_catalogo', 'nombre', 'nombre_etiqueta', 'descripcion', 'caracteristicas',
        'categoria', 'estado_bien', 'marca', 'modelo', 'color', 'medidas',
        'medida_largo', 'medida_ancho', 'medida_altura', 'observacion',
  ...
  IF p_patch ? 'codigo_catalogo' THEN
    IF trim(coalesce(p_patch->>'codigo_catalogo', '')) = '' THEN
      RAISE EXCEPTION 'Código catálogo obligatorio';
    END IF;
    IF NOT EXISTS (
      SELECT 1 FROM public.catalogo_nacional
      WHERE codigo = trim(p_patch->>'codigo_catalogo')
    ) THEN
      RAISE EXCEPTION 'Código de catálogo no válido';
    END IF;
  END IF;

  IF p_patch ? 'nombre' AND trim(coalesce(p_patch->>'nombre', '')) = '' THEN
  ...
  SET
    codigo_catalogo = CASE WHEN p_patch ? 'codigo_catalogo'
      THEN trim(p_patch->>'codigo_catalogo') ELSE a.codigo_catalogo END,
    nombre = CASE WHEN p_patch ? 'nombre'
      THEN trim(p_patch->>'nombre') ELSE a.nombre END,
    nombre_etiqueta = CASE WHEN p_patch ? 'nombre_etiqueta'
      THEN NULLIF(trim(p_patch->>'nombre_etiqueta'), '') ELSE a.nombre_etiqueta END,
    descripcion = CASE WHEN p_patch ? 'descripcion'
      THEN NULLIF(trim(p_patch->>'descripcion'), '') ELSE a.descripcion END,
    ...
    medida_largo = CASE WHEN p_patch ? 'medida_largo'
      THEN NULLIF(p_patch->>'medida_largo', '')::NUMERIC ELSE a.medida_largo END,
    medida_ancho = CASE WHEN p_patch ? 'medida_ancho'
      THEN NULLIF(p_patch->>'medida_ancho', '')::NUMERIC ELSE a.medida_ancho END,
    medida_altura = CASE WHEN p_patch ? 'medida_altura'
      THEN NULLIF(p_patch->>'medida_altura', '')::NUMERIC ELSE a.medida_altura END,
...
GRANT EXECUTE ON FUNCTION public.update_activos_similares(UUID, JSONB) TO authenticated;
