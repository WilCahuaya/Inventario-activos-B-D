-- Mejora / componente: detalle e incremento; valor efectivo = adquisición + incremento.

ALTER TABLE public.activos
  ADD COLUMN IF NOT EXISTS incremento_detalle TEXT,
  ADD COLUMN IF NOT EXISTS valor_incremento NUMERIC(14, 2);
...
DECLARE
  v_fields TEXT[] := ARRAY[
    ...
    'incremento_detalle', 'valor_incremento',
...
AS $$
  SELECT
    ...
    AND coalesce(trim(a.incremento_detalle), '') = coalesce(trim(t.incremento_detalle), '')
    AND a.valor_incremento IS NOT DISTINCT FROM t.valor_incremento
...
BEGIN
  ...
  FOR v_i IN 1..p_cantidad LOOP
    ...
    VALUES (
      ...
      v_template.incremento_detalle,
      v_template.valor_incremento,
...
BEGIN
  ...
  SET
    ...
    incremento_detalle = CASE WHEN p_patch ? 'incremento_detalle'
      THEN NULLIF(trim(p_patch->>'incremento_detalle'), '') ELSE a.incremento_detalle END,
    valor_incremento = CASE WHEN p_patch ? 'valor_incremento'
      THEN NULLIF(p_patch->>'valor_incremento', '')::NUMERIC ELSE a.valor_incremento END,
