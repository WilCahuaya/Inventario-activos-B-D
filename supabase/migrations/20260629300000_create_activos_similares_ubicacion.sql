-- Permite crear bienes similares en un ambiente distinto al de la plantilla.
-- Hay que DROP: CREATE OR REPLACE no puede añadir parámetros a una función existente.

DROP FUNCTION IF EXISTS public.create_activos_similares(UUID, INTEGER);
DROP FUNCTION IF EXISTS public.create_activos_similares(UUID, INTEGER, UUID, UUID);

CREATE FUNCTION public.create_activos_similares(
  p_activo_id UUID,
  p_cantidad INTEGER,
...
GRANT EXECUTE ON FUNCTION public.create_activos_similares(UUID, INTEGER, UUID, UUID) TO authenticated;
